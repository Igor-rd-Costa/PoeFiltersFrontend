import { AfterViewInit, Component, computed, ElementRef, Input, model, signal, ViewChild } from '@angular/core';
...
import { Color, ColorRGBA, FilterRuleType, FilterRule, RuleStyle } from '../../types/FilterTypes';
import FilterHelpers from '../../utils/FilterHelpers';

@Component({
  selector: 'app-filter-rule',
  standalone: true,
  imports: [ColorInputComponent, IconInputComponent, SoundInputComponent, PlayEffectInputComponent, 
    BaseTypeInputComponent, FontSizeInputComponent, RuleSettingsFormComponent],
...
export class FilterRuleComponent implements AfterViewInit {
  ...
  rule = model.required<FilterRule>();
  ruleStyle = computed(() => {
    const r = this.rule();
  ...
  protected ResetStyles() {
    this.rule().style = FilterHelpers.DefaultRuleStyle();
    this.UpdatePreviewStyle();
  }
  ...
  private UpdatePreviewStyle() {
    ...
    let stl = {
      fontSize: fs + 'px',
      color: (tx !== null && tx.active) ? `rgba(${tx.r}, ${tx.g}, ${tx.b}, ${tx.a})` : 'rgba(220, 220, 220, 1)',
      borderColor: (bc !== null && bc.active) ? `rgba(${bc.r}, ${bc.g}, ${bc.b}, ${bc.a})` : 'transparent',
      backgroundColor: (bgC !== null && bgC.active) ? `rgba(${bgC.r}, ${bgC.g}, ${bgC.b}, ${bgC.a})` : 'rgba(0,0,0, 0.7)',
    };
